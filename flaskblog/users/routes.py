...
from flaskblog.users.forms import RegistrationForm, LoginForm, UpdateAccountForm, RequestToken, CreateNewPassword
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(
            f"Account succesfully created for {form.username.data}!", "success")
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('users.login'))
    return render_template('verify_token.html', title='Reset Password', form=form)
